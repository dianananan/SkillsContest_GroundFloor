...
u8 Normal_data(u8 mode, u8 *buff)	 	  // 正常接收8字节控制指令
{
	u8 sum=0;
...
//	}
    ...
    if(buff[0] == 0x55 && buff[7] == 0xbb)	 // 判断包尾
    {
        sum = (buff[2] + buff[3] + buff[4] + buff[5]) % 256; //注意：在求和溢出时应该对和做256取余。  //主指令与三位副指令左求和校验
        if(sum == buff[6])
        {
            Rx_Flag = mode;
        }
    ...
    {
        sum = (buff[1] + buff[2] + buff[3] + buff[4] + buff[5]) % 256;
        if(sum == buff[6])
        {
            Rx_Flag = mode;
        }
...
//			case 0x02:              //前进
//				Roadway_mp_syn();	//码盘同步
//				Stop_Flag = 0; G_Flag = 1; L_Flag = 0;R_Flag = 0;wheel_Nav_Flag = 0;
//				B_Flag = 0; Track_Flag = 0;
...
//			case 0x03:              //后退
//				Roadway_mp_syn();	//码盘同步
//				Stop_Flag = 0; G_Flag = 0; L_Flag = 0;R_Flag = 0;wheel_Nav_Flag = 0;
//				B_Flag = 1; Track_Flag = 0;
//				temp_MP = Wifi_Rx_Buf[5];
//				temp_MP <<=  8;
//				temp_MP |= Wifi_Rx_Buf[4];
//				Car_Spend = Wifi_Rx_Buf[3];
...
//				break;
//			case 0x04:              //左转
//				Stop_Flag = 0; G_Flag = 0; L_Flag = 1;R_Flag = 0;wheel_Nav_Flag = 0;
//				B_Flag = 0; Track_Flag = 0;
//				Car_Spend = Wifi_Rx_Buf[3];				
//				Control(-Car_Spend ,Car_Spend);
//				break;
//			case 0x05:              //右转
//				Stop_Flag = 0; G_Flag = 0; L_Flag = 0;R_Flag = 1;wheel_Nav_Flag = 0;
//				B_Flag = 0; Track_Flag = 0;
//				Car_Spend = Wifi_Rx_Buf[3];
//				Control(Car_Spend,-Car_Spend);
//				break;
...
//				Roadway_mp_syn();	//码盘同步
//				Stop_Flag = 0; G_Flag = 0; L_Flag = 0;R_Flag = 0;wheel_Nav_Flag = 1;
//				B_Flag = 0; Track_Flag = 0;
...
//				Infrared_Send(H_N[1],4);
//				break;
//			case 0x63:    					//红外发射控制光源强度档位加3 
//				Infrared_Send(H_N[2],4);
//				break;
...
/*	if(Zigbee_Rx_flag == 0)
   ...
   if(Zigbee_Rx_num >= 8 && gt_get_sub(canu_zibe_rxtime))
	{	
		if(Zigb_Rx_Buf[0]==0x55 && Zigb_Rx_Buf[7] == 0xbb)	//校验包头包尾
		{
			if(Normal_data(2,Zigb_Rx_Buf))
			{
				Zigbee_Rx_flag=1;
			}
		}
