...
#define HW_LIGHT 		0x12			//光源调档
#define HW_LIGHTRE		0x13			//返回光源当前挡位
#define HW_PLATESHOW	0x14			//立体显示车牌
#define HW_TENNEL		0x15			//隧道排风
#define HW_PICUP		0x16			//图片上翻
#define	HW_PICDOWN		0x17			//图片下翻
#define HW_OPENBJQ		0x18			//打开报警器
#define	HW_CLOSEBJQ		0x19			//关闭报警器
#define HW_TYPESHOW		0x1A			//立体显示图形
#define STOP_THIS		0x1B			//停止  
#define HW_DISSHOW		0x1C			//立体显示距离
#define HW_HUESHOW		0x1D			//立体显示颜色
#define HW_LKSHOW		0x1E			//立体显示路况
#define HW_LASTSHOW		0x1F			//立体显示默认
//#define HW_DISTANCE					CMD_DISTANCE 		//发送距离到立体显示 （18）

#define DELAY			0x20			//延时
#define LED_L_OPEN		0x21			//打开左边转向灯
#define LED_L_CLOSE		0x22			//关闭左边转向灯
#define LED_R_OPEN		0x23			//打开右边转向灯
#define LED_R_CLOSE		0x24			//关闭右边转向灯
#define LED_OPEN		0x25			//打开左右转向灯
#define LED_CLOSE		0x26			//关闭左右转向灯
#define BEEP_OPEN		0x27			//打开蜂鸣器
#define BEEP_CLOSE		0x28			//关闭蜂鸣器
#define CMD_GETPOINT	0x29			//超声波探距
#define UPRIGHT			0x2A			//矫正当前点
...
//static u8 SD_CPF[6]={0xFF,0x20,0x00,0x00,0x00,0x00};   //接收前四位车牌信息模式（后四位）
//static u8 SD_CPB[6]={0xFF,0x10,0x00,0x00,0x00,0x00};   //接收后两位车牌信息模式（前两位车牌数据）（后两位 横坐标，纵坐标）
static u8 SD_JL[6] ={0xFF,0x11,0x38,0x39,0x00,0x00};   //显示距离（十位，个位，00，00）
static u8 CP_G1[6] ={0xFF,0x12,0x02,0x00,0x00,0x00};   //显示图像（数据，00，00，00）
static u8 CP_G2[6] ={0xFF,0x13,0x00,0x00,0x00,0x00};   //显示颜色（数据，00，00，00）
static u8 SD_LK[6] ={0xFF,0x14,0x00,0x00,0x00,0x00};   //显示路况（01有事故/02施工，00，00，00）
static u8 SD_MRXX[6]={0xFF,0x15,0x01,0x00,0x00,0x00};   //显示默认
...
static u8 MAGLEV[8]={0x55,0x0a,0x01,0x01,0x00,0x00,0x02,0xbb};//磁悬浮

static u8 SMG_SHOW[8]={0x55,0x04,0x01,0x00,0x00,0x00,0x01,0xBB};		//数码管第一排显示
static u8 SMG_SHOWTWO[8]={0x55,0x04,0x02,0x01,0x00,0x00,0x03,0xBB};		//数码管第二排显示 
...
static u8 QRCode[SENDNUM] = {0x55,0xAA,0x02,0x00,0x00,0x00,0x02,0xbb};//二维码到达点发送指令
static u8 PlateRead[SENDNUM] = {0x55,0xAA,0x03,0x00,0x00,0x00,0x03,0xbb};//车牌到达点发送指令
static u8 ShapeRead[SENDNUM] = {0x55,0xAA,0x04,0x00,0x00,0x00,0x04,0xbb};//图形识别到达点发送指令
static u8 TrafficRead[SENDNUM]= {0x55,0xAA,0x05,0x00,0x00,0x00,0x05,0xbb};//交通灯到达点发送指令
static u8 TrafficLight[SENDNUM]={0x55,0xAA,0x07,0x00,0x00,0x00,0x07,0xbb};//智能红绿灯到达发送指令
static u8 GetPoint[SENDNUM] = {0x55,0xAA,0x06,0x00,0x00,0x00,0x06,0xbb};//发送得到入库点指令
...
//static u8 AVG_RETURN[SENDNUM] = {0x55,0xAA,0xEE,0x00,0x00,0x00,0xEE,0xbb};

static u8 CP_SHOW1[6] = {0xFF, 0x20, 65, 66, 67, 68};
static u8 CP_SHOW2[6] = {0xFF, 0x10, 69, 70, 71, 72}; //立体车牌显示
...
extern u8 BJT_flag[6];

extern u8 C_Tab[10];	//返回状态数组																															
extern int light;
extern u32 counttime;
...
u8 CalParity(u8 the1,u8 the2 ,u8 the3 ,u8 the4);		//计算校验和
extern u8 getNEWTask(void);
void RFIDPotRefesh(void);
